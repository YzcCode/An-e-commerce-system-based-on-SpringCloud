    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="isSaleable(props.item)">下架</v-btn>
    ...
    methods: {
      ...
      deleteGoods(oldGoods){
        this.$message.confirm("确认要删除该商品吗？")
          .then(() => {
            this.$http.delete("/item/spu/" + oldGoods.id)
              .then(() => {
      ...
      isSaleable(oldGoods){
        ...
        this.$message.confirm("确认要"+s+"该商品吗？")
          .then(() => {
            this.$http.get("/item/spu/" + oldGoods.id +"/" + oldGoods.saleable)
              .then(() => {
