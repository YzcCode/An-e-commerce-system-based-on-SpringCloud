package com.lingyang.order.controller;

import com.lingyang.common.pojo.PageResult;
import com.lingyang.item.pojo.Brand;
import com.lingyang.order.pojo.Order;
import com.lingyang.order.service.OrderService;
import com.lingyang.utils.PayHelper;
import com.lingyang.utils.PayState;
...
public class OrderController {
     ...
     */
    @GetMapping("listPage")
    @ApiOperation(value = "分页查询所有订单，并且可以根据订单状态过滤", notes = "分页查询所有订单")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "page", value = "当前页", defaultValue = "1", type = "Integer"),
            @ApiImplicitParam(name = "rows", value = "每页大小", defaultValue = "5", type = "Integer"),
            @ApiImplicitParam(name = "keys", value = "搜索条件", type = "String"),
            @ApiImplicitParam(name = "status", value = "订单状态：1未付款，2已付款未发货，3已发货未确认，4已确认未评价，5交易关闭，6交易成功，已评价", type = "Integer"),
    })
    @ApiResponses({
            @ApiResponse(code = 200, message = "订单的分页结果"),
    ...
    public ResponseEntity<PageResult<Order>> queryUserOrderList(
        ...
        PageResult<Order> result = this.orderService.queryOrderList(page, rows, keys, status);
        if (result == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        return ResponseEntity.ok(result);
     ...
     */
    @PutMapping("update")
    public ResponseEntity<Void> updataOrder(@RequestBody Order order){
        this.orderService.updataOrder(order);
